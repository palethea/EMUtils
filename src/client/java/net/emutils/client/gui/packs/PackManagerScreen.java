...
import net.emutils.client.packs.InstalledPack;
import net.emutils.client.packs.InstalledPackIndex;
import net.emutils.client.packs.InstalledPackScanner;
import net.emutils.client.packs.PackOperationResult;
import net.emutils.client.packs.PackType;
import net.emutils.client.packs.ResourcePackController;
...
public final class PackManagerScreen extends Screen {
	...
	private record RefreshResult(List<PackListItem> installed, @Nullable List<PackListItem> search) {
	}
	...
	protected void init() {
		...
		resourceTab = addDrawableChild(ButtonWidget.builder(Text.translatable(EMUtilsTexts.PACK_TAB_RESOURCE_PACKS), button -> switchType(PackType.RESOURCE)).width(TAB_WIDTH).build());
		shaderTab = addDrawableChild(ButtonWidget.builder(Text.translatable(EMUtilsTexts.PACK_TAB_SHADER_PACKS), button -> switchType(PackType.SHADER)).width(TAB_WIDTH).build());
		installedTab = addDrawableChild(ButtonWidget.builder(Text.translatable(EMUtilsTexts.PACK_TAB_INSTALLED), button -> switchView(ViewMode.INSTALLED)).width(TAB_WIDTH).build());
		modrinthTab = addDrawableChild(ButtonWidget.builder(Text.translatable(EMUtilsTexts.PACK_TAB_MODRINTH), button -> switchView(ViewMode.MODRINTH)).width(TAB_WIDTH).build());

		searchField = addDrawableChild(new TextFieldWidget(textRenderer, 0, 0, SEARCH_FIELD_WIDTH, TAB_HEIGHT, Text.translatable(EMUtilsTexts.PACK_SEARCH_PLACEHOLDER)));
		searchField.setPlaceholder(Text.translatable(EMUtilsTexts.PACK_SEARCH_PLACEHOLDER).formatted(Formatting.GRAY));
		searchButton = addDrawableChild(GalleryIconButtonWidget.create(Text.translatable(EMUtilsTexts.PACK_SEARCH), PackIcons.SEARCH, PackIcons.SIZE, button -> search()));

		list = addDrawableChild(new PackSearchWidget(client, width, height, this::download, this::enable, this::disable, this::delete, this::applyShader, this::disableShader));
	...
	protected void refreshWidgetPositions() {
		...
		int listHeight = Math.max(40, height - y - layout.getFooterHeight() - 8);
		if (list != null) {
	...
	public void tick() {
		super.tick();
		if (viewMode == ViewMode.INSTALLED && ++installedPollTicks >= INSTALLED_POLL_TICKS) {
	...
	public boolean keyPressed(net.minecraft.client.input.KeyInput input) {
		if (viewMode == ViewMode.MODRINTH && searchField != null) {
			if ((input.key() == 257 || input.key() == 335) && searchField.isFocused()) {
				search();
				return true;
			}
	...
	private void refreshTabState() {
		...
		if (shaderTab != null) {
			shaderTab.active = activeType != PackType.SHADER && (IrisCompat.isIrisLoaded() || EMUtilsClient.config().packManagerShowShadersWithoutIris());
		}
	...
	private void pollInstalled() {
		...
		CompletableFuture.supplyAsync(() -> scanInstalled(type), EXECUTOR).whenComplete((items, throwable) -> client.execute(() -> {
	...
	private void loadModrinth(String query) {
		if (!EMUtilsClient.config().packManagerEnabled()) {
			showMessage(Text.translatable(EMUtilsTexts.OPTION_PACK_MANAGER).formatted(Formatting.RED));
			return;
		}
		...
		int requestId = ++refreshRequestId;
		PackType type = activeType;
		CompletableFuture.supplyAsync(() -> new RefreshResult(scanInstalled(type), loadSearchItems(type, query)), EXECUTOR)
	...
	private void refreshAfterChange() {
		...
		boolean reloadSearch = viewMode == ViewMode.MODRINTH && searchItems != null;
		CompletableFuture.supplyAsync(() -> {
			List<PackListItem> installed = scanInstalled(type);
			List<PackListItem> search = reloadSearch ? loadSearchItems(type, query) : null;
			return new RefreshResult(installed, search);
		}, EXECUTOR).whenComplete((result, throwable) -> client.execute(() -> {
			if (requestId != refreshRequestId) {
				return;
			}
			if (throwable != null) {
	...
	private List<PackListItem> scanInstalled(PackType type) {
		try {
			return InstalledPackScanner.scan(client, type, index).stream()
	...
	private List<PackListItem> loadSearchItems(PackType type, String query) {
		try {
			List<InstalledPack> installed = InstalledPackScanner.scan(client, type, index);
			...
			for (ModrinthSearchResult result : results) {
				InstalledPack matched = installed.stream()
					.filter(pack -> pack.record() != null && pack.record().matches(type, result.projectId()))
					.findFirst()
					.orElse(null);
	...
	private void download(PackListItem item) {
		...
		CompletableFuture.supplyAsync(() -> {
			try {
				ResourcePackController.install(modrinth, client, item.type(), item.result(), index);
			...
			showMessage(Text.literal(result.message()).formatted(result.success() ? Formatting.GREEN : Formatting.RED));
			if (result.success()) {
				refreshAfterChange();
			}
		}));
	...
	private void enable(PackListItem item) {
		...
		PackOperationResult result = ResourcePackController.setResourcePackEnabled(client, item.installed().filename(), true);
		showMessage(Text.literal(result.message()).formatted(result.success() ? Formatting.GREEN : Formatting.RED));
		refreshAfterChange();
	}
	...
	private void disable(PackListItem item) {
		...
		PackOperationResult result = ResourcePackController.setResourcePackEnabled(client, item.installed().filename(), false);
		showMessage(Text.literal(result.message()).formatted(result.success() ? Formatting.GREEN : Formatting.RED));
		refreshAfterChange();
	}
	...
	private void delete(PackListItem item) {
		...
		client.setScreen(new ConfirmScreen(
			...
			Text.translatable(EMUtilsTexts.PACK_DELETE_MESSAGE, item.installed().filename()),
	...
	private void deleteConfirmed(InstalledPack pack) {
		if (pack.type() == PackType.RESOURCE && pack.enabled()) {
			PackOperationResult disabled = ResourcePackController.setResourcePackEnabled(client, pack.filename(), false);
			if (!disabled.success()) {
				showMessage(Text.literal(disabled.message()).formatted(Formatting.RED));
				return;
			}
		}
		CompletableFuture.supplyAsync(() -> ResourcePackController.deleteInstalledFile(client, index, pack), EXECUTOR)
	...
	private void applyShader(PackListItem item) {
		...
		if (!IrisCompat.isIrisLoaded()) {
			showMessage(Text.translatable(EMUtilsTexts.PACK_STATUS_IRIS_REQUIRED).formatted(Formatting.RED));
			return;
		}

		IrisCompat.applyShaderPackWithLoading(client, this, item.installed().filename(), success -> {
			if (success) {
				showMessage(Text.translatable(EMUtilsTexts.PACK_SHADER_APPLIED, item.title()).formatted(Formatting.GREEN));
			} else {
				showMessage(Text.translatable(EMUtilsTexts.PACK_SHADER_APPLY_FAILED, item.title()).formatted(Formatting.RED));
			}
			refreshAfterChange();
		});
	}
	...
	private void disableShader(PackListItem item) {
		...
		IrisCompat.disableShaderPackWithLoading(client, this, success -> {
			if (success) {
				showMessage(Text.translatable(EMUtilsTexts.PACK_SHADER_DISABLED, item.title()).formatted(Formatting.GREEN));
			} else {
				showMessage(Text.translatable(EMUtilsTexts.PACK_SHADER_DISABLE_FAILED, item.title()).formatted(Formatting.RED));
			}
			refreshAfterChange();
		});
	}
	...
	private void updateList() {
		...
		if (viewMode == ViewMode.INSTALLED) {
			list.setItems(installedItems, PackSearchWidget.ViewMode.INSTALLED, installedEmptyHint());
			return;
		}

		if (searchItems == null) {
			list.setItems(List.of(), PackSearchWidget.ViewMode.MODRINTH, Text.empty());
			return;
		}

		list.setItems(searchItems, PackSearchWidget.ViewMode.MODRINTH, modrinthEmptyHint());
	}

	private Text installedEmptyHint() {
		return Text.translatable(activeType == PackType.SHADER
	...
	private void setError(Throwable throwable) {
		...
		showMessage(Text.translatable(EMUtilsTexts.PACK_ERROR, cause.getMessage()).formatted(Formatting.RED));
		if (viewMode == ViewMode.MODRINTH) {
			searchItems = List.of();
			updateList();
		}
	...
	private void showMessage(Text message) {
		client.inGameHud.getChatHud().addMessage(message, null, null);
	}
}
