...
public final class DeathWaypointListScreen extends Screen {
	private final Screen parent;
	private ThreePartsLayoutWidget layout;
	private DeathWaypointListWidget list;
	private ButtonWidget clearButton;

	public DeathWaypointListScreen(Screen parent) {
	...
	protected void init() {
		...
		footer.add(ButtonWidget.builder(ScreenTexts.DONE, button -> close()).width(150).build());
		...
		list = addDrawableChild(new DeathWaypointListWidget(this, client, width, height));
		refreshList();
	...
	void refreshList() {
		...
		if (clearButton != null) {
			clearButton.active = EMUtilsClient.deathWaypoint().hasWaypointForCurrentWorld(client);
		}
	}
