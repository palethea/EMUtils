...
import net.emutils.client.gui.death.DeathWaypointListScreen;
...
public final class HubSettingsRegistry {
	...
	private static List<HubSettingRow> deathRows(Runnable refresh) {
		EMUtilsConfig config = config();
		List<HubSettingRow> rows = new ArrayList<>();
		rows.add(new HubSettingRow.Toggle(EMUtilsTexts.OPTION_DEATH_WAYPOINT, config::deathWaypoint, config::setDeathWaypoint));
		rows.add(new HubSettingRow.Toggle(EMUtilsTexts.OPTION_DEATH_WAYPOINT_AUTO_COPY, config::deathWaypointAutoCopyCoords, config::setDeathWaypointAutoCopyCoords));
		rows.add(new HubSettingRow.Cycle<>(
			EMUtilsTexts.OPTION_DEATH_COORD_FORMAT,
			config::deathWaypointCoordinateFormat,
			config::setDeathWaypointCoordinateFormat,
			() -> config.deathWaypointCoordinateFormat().next(),
			() -> Text.translatable(config.deathWaypointCoordinateFormat().labelKey())
		));
		rows.add(divider());
		rows.add(new HubSettingRow.Slider(
			...
			config::deathWaypointOpacity,
			config::setDeathWaypointOpacity
		));
		rows.add(new HubSettingRow.Slider(
			...
			config::deathWaypointSize,
			config::setDeathWaypointSize
		));
		rows.add(divider());
		rows.add(navRow(EMUtilsTexts.OPTION_CURRENT_WAYPOINTS, DeathWaypointListScreen::new));
		return rows;
	}
	...
	private static List<HubSettingRow> inventoryRows(Runnable refresh) {
		...
		rows.add(new HubSettingRow.Slider(
			EMUtilsTexts.OPTION_INVENTORY_PREVIEW_OPACITY,
			EMUtilsTexts.SUFFIX_PERCENT,
			EMUtilsConfig.INVENTORY_PREVIEW_OPACITY_MIN,
			EMUtilsConfig.INVENTORY_PREVIEW_OPACITY_MAX,
			config::inventoryPreviewOpacity,
			config::setInventoryPreviewOpacity
		));
		return rows;
	}
