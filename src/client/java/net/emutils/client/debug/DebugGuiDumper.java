package net.emutils.client.debug;
...
import net.emutils.client.skyblock.ItemStackSerializer;
...
public final class DebugGuiDumper {
	...
	public static void capture(MinecraftClient client) {
		...
		try {
			...
			root.addProperty("world", client.world == null ? null : client.world.getRegistryKey().getValue().toString());
			...
			if (client.player != null) {
				client.player.sendMessage(Text.literal("Debug dump saved, path copied."), true);
			}
			EMUtilsClient.LOGGER.info("Saved GUI debug dump to {}.", path);
		} catch (Exception exception) {
			EMUtilsClient.LOGGER.warn("Failed to save GUI debug dump.", exception);
			if (client.player != null) {
				client.player.sendMessage(Text.literal("Debug dump failed."), true);
			}
		}
	}
	...
	private static JsonObject serializeScreen(MinecraftClient client) {
		...
		object.addProperty("titlePlain", SkyblockTextUtils.strip(screen.getTitle()));

		if (screen instanceof HandledScreen<?> handledScreen) {
			ScreenHandler handler = handledScreen.getScreenHandler();
			object.addProperty("syncId", handler.syncId);
			object.add("focusedSlot", serializeSlot(client, ((HandledScreenAccessor) handledScreen).emutils$getFocusedSlot()));
	...
	private static JsonArray serializeSlots(MinecraftClient client, ScreenHandler handler) {
	...
	private static JsonObject serializeSlot(MinecraftClient client, @Nullable Slot slot) {
		JsonObject object = new JsonObject();
		if (slot == null) {
		...
		object.addProperty("inventoryType", slot.inventory.getClass().getName());
		object.addProperty("playerInventory", slot.inventory instanceof PlayerInventory);
		...
		object.addProperty("namePlain", SkyblockTextUtils.strip(stack.getName()));
	...
	private static JsonArray tooltipJson(MinecraftClient client, ItemStack stack) {
	...
	private static Path writeFile(String json) throws IOException {
		...
		Path path = directory.resolve("gui-" + System.currentTimeMillis() + ".json");
