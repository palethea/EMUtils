...
public final class SeaCreatureTrackerManager {
    ...
    public static void resetCurrentMode() {
        if (displayMode == TrackerDisplayMode.SESSION) {
            FishingTrackerStorage.resetSessionSeaCreature();
        } else {
            FishingTrackerStorage.resetAllTimeSeaCreature();
        }
    }
    ...
    public static boolean onSeaCreatureChat(
        String cleanMessage,
        boolean recordTracker
    ) {
        SeaCreatureDefinition creature = SeaCreatureRegistry.fromChat(
            cleanMessage
        );
        if (creature == null) {
            if (
                pendingDoubleHook &&
                System.currentTimeMillis() - pendingDoubleHookMs >
                    DOUBLE_HOOK_TIMEOUT_MS
            ) {
        ...
        int amount =
            pendingDoubleHook &&
            EMSkyblockSettings.seaCreatureTrackerCountDouble()
                ? 2
                : 1;
        pendingDoubleHook = false;
        pendingDoubleHookMs = 0L;
    ...
    private static void recordCatch(String creatureId, int amount) {
        FishingTrackerStorage.seaCreature(TrackerDisplayMode.SESSION).add(
            creatureId,
            amount
        );
        FishingTrackerStorage.seaCreature(TrackerDisplayMode.ALL_TIME).add(
            creatureId,
            amount
        );
    ...
    public static boolean shouldShow(MinecraftClient client) {
        if (
            ...
            !EMSkyblockSettings.seaCreatureTrackerEnabled()
        ) {
            return false;
        }
        ...
        if (
            EMSkyblockSettings.seaCreatureTrackerShowWithFishingArmor() &&
        ...
        if (
            EMSkyblockSettings.seaCreatureTrackerShowOnFishingIslands() &&
    ...
    public static List<TrackerPanelLine> lines(
        ...
        lines.add(
            TrackerPanelLine.header(
                TrackerPanelLine.TrackerHeaderParts.seaCreature(displayMode)
            )
        );
        ...
        data.counts
            ...
            .forEach(entry -> {
                SeaCreatureDefinition definition = SeaCreatureRegistry.byId(
                    entry.getKey()
                );
                String name =
                    definition == null
                ...
                String color =
                    definition == null ? "§e" : definition.chatColor();
                String suffix = "";
                if (
                    EMSkyblockSettings.seaCreatureTrackerShowPercentage() &&
                    total > 0
                ) {
                ...
                String rare = (definition != null && definition.rare())
                    ? "§l"
                    : "";
                lines.add(
                    TrackerPanelLine.of(
                        " §7› §e" +
                            formatCount(entry.getValue()) +
                            " " +
                            color +
                            rare +
                            name +
        ...
        lines.add(
            TrackerPanelLine.of(
                " §7› §e" + formatCount(total) + " §7Total Sea Creatures"
            )
        );
        if (
            EMSkyblockSettings.seaCreatureTrackerShowUptime() &&
