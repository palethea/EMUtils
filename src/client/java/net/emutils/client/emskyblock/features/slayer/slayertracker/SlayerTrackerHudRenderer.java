...
public final class SlayerTrackerHudRenderer {
	private static final Identifier ID = Identifier.of(
		EMUtilsClient.MOD_ID,
		"slayer_profit_tracker"
	);

	private SlayerTrackerHudRenderer() {}
	...
	public static List<TrackerPanelLine> previewLines() {
		return SlayerTrackerManager.lines(MinecraftClient.getInstance(), true);
	}
	...
	private static void render(DrawContext context) {
		...
		TrackerHudHitbox.clear(HudElementId.SLAYER_PROFIT_TRACKER);
		if (config == null || client.player == null || client.world == null) {
			return;
		}

		boolean editing = HudLayoutManager.isEditing();
		if (!SlayerTrackerManager.shouldShow(client) && !editing) {
			return;
		}

		if (
			EMUtilsClient.zoom() != null &&
			EMUtilsClient.zoom().shouldHideHud() &&
			!editing
		) {
			return;
		}
		...
		List<TrackerPanelLine> lines = SlayerTrackerManager.lines(client, false);
		if (lines.isEmpty()) {
			return;
		}

		HudLayoutManager.ResolvedLayout layout = HudLayoutManager.resolveLayout(
			HudElementId.SLAYER_PROFIT_TRACKER,
			config,
		...
		TrackerDisplayMode mode = SlayerTrackerManager.displayMode();
		if (client.mouse != null && client.getWindow() != null) {
			...
			if (
				TrackerPanelRenderer.isModeClickHovered(
					layout.position().x(),
					layout.position().y(),
					layout.scaleFactor(),
					client.textRenderer,
				...
				lines.add(
					TrackerPanelLine.of(
						TrackerPanelLine.TrackerHeaderParts.tooltipLine(mode)
					)
				);
			}
		}
