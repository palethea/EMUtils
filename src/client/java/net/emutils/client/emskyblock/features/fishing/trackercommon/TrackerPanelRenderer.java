...
public final class TrackerPanelRenderer {
	...
	public static boolean isModeClickHovered(
		int panelScreenX,
		int panelScreenY,
		float scaleFactor,
		TextRenderer textRenderer,
		...
		int scaledModeX = panelScreenX + Math.round(localModeX * scaleFactor);
		int scaledModeY = panelScreenY + Math.round(localModeY * scaleFactor);
		int scaledModeW = Math.round(modeClickWidth * scaleFactor);
		int scaledModeH = Math.round(ROW_HEIGHT * scaleFactor);

		return mouseX >= scaledModeX
			&& mouseX < scaledModeX + scaledModeW
			&& mouseY >= scaledModeY
			&& mouseY < scaledModeY + scaledModeH;
	}
