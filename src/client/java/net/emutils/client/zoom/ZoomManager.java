package net.emutils.client.zoom;
...
public final class ZoomManager {
	...
	private static final float ZOOM_OUT_SPEED_MULTIPLIER = 1.75F;
	...
	public boolean wantsZoom(MinecraftClient client) {
	...
	public boolean isZoomEffectActive() {
		updateAnimation(MinecraftClient.getInstance());
	...
	public boolean shouldUseCinematicCamera() {
		...
		return wantsZoom(MinecraftClient.getInstance());
	}
	...
	private void updateAnimation(MinecraftClient client) {
		if (EMUtilsClient.config() == null || !EMUtilsClient.config().zoomEnabled()) {
			targetDivisor = 1.0F;
			currentDivisor = 1.0F;
			lastUpdateNanos = 0L;
		...
		if (targetDivisor < currentDivisor) {
			speed *= ZOOM_OUT_SPEED_MULTIPLIER;
		}
