...
public final class FoodTooltipComponent implements TooltipComponent {
	...
	private void drawHungerRow(TextRenderer textRenderer, DrawContext context, int x, int y) {
		...
		for (int i = 0; i < hungerBars * 2; i += 2) {
			context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, FoodHudTextures.empty(rotten), drawX, y, HUNGER_ICON_SIZE, HUNGER_ICON_SIZE, 0.45F);
			if (modifiedHunger > i) {
				boolean half = modifiedHunger - 1 == i;
				context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, half ? FoodHudTextures.half(rotten) : FoodHudTextures.full(rotten), drawX, y, HUNGER_ICON_SIZE, HUNGER_ICON_SIZE);
			} else if (defaultHunger > i) {
				boolean half = defaultHunger - 1 == i;
				context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, half ? FoodHudTextures.half(rotten) : FoodHudTextures.full(rotten), drawX, y, HUNGER_ICON_SIZE, HUNGER_ICON_SIZE, 0.25F);
			}
			drawX -= HUNGER_ICON_SIZE;
		}
		if (hungerText != null) {
			context.drawText(textRenderer, Text.literal(hungerText), x + HUNGER_ICON_SIZE + 2, y + 1, 0xFFAAAAAA, false);
		}
	}

	private void drawSaturationRow(TextRenderer textRenderer, DrawContext context, int x, int y) {
		...
		for (int i = 0; i < saturationBars * 2; i += 2) {
			...
			context.drawTexture(RenderPipelines.GUI_TEXTURED, FoodHudTextures.ICONS, drawX, y, u, v, SATURATION_ICON_SIZE, SATURATION_ICON_SIZE, SATURATION_ICON_SIZE, SATURATION_ICON_SIZE, 256, 256, color);
			drawX -= SATURATION_ICON_SIZE;
		}
		if (saturationText != null) {
			context.drawText(textRenderer, Text.literal(saturationText), x + SATURATION_ICON_SIZE + 2, y, 0xFFAAAAAA, false);
		}
	}

	private static int textWidth(TextRenderer textRenderer, String text) {
		return text == null ? 0 : textRenderer.getWidth(text) + 2;
	}
}
