...
public final class PythonScriptEditorWidget extends ClickableWidget {
	...
	public PythonScriptEditorWidget(MinecraftClient client, int x, int y, int width, int height, Runnable dirtyListener) {
	...
	protected void renderWidget(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
		...
		if (isFocused()) {
			int caretX = contentX + textRenderer.getWidth(lines.get(caretLine).substring(0, Math.min(caretColumn, lines.get(caretLine).length()))) - horizontalScroll;
	...
	private void renderLineNumber(DrawContext context, int lineIndex, int gutterRight, int lineY) {
		String label = String.valueOf(lineIndex + 1);
		int color = lineIndex == caretLine && isFocused() ? 0xFFCCCCCC : 0xFF707070;
		int textX = gutterRight - textRenderer.getWidth(label) - 2;
		context.drawText(textRenderer, label, textX, lineY, color, false);
	}

	@Override
	protected void appendClickableNarrations(NarrationMessageBuilder builder) {
		builder.put(NarrationPart.TITLE, Text.literal("Python script editor"));
	}

	private void renderLine(DrawContext context, String line, int x, int y) {
		int drawX = x;
		int index = 0;
	...
	private int drawSegment(DrawContext context, String segment, int x, int y, int color) {
		context.drawText(textRenderer, segment, x, y, color, false);
		return x + textRenderer.getWidth(segment);
	}

	private void renderSelection(DrawContext context, int lineIndex, int x, int y) {
		if (!hasSelection()) {
			return;
		}
		...
		int selectionStartX = x + textRenderer.getWidth(line.substring(0, Math.min(startColumn, line.length())));
		int selectionEndX = x + textRenderer.getWidth(line.substring(0, Math.min(endColumn, line.length())));
	...
	public boolean mouseClicked(Click click, boolean doubled) {
		...
		if (isInLineNumberGutter(click.x())) {
			...
			setCaret(line, 0, click.hasShift());
			dragging = true;
			return true;
		}
		moveCaretToMouse(click.x(), click.y(), click.hasShift());
		dragging = true;
		return true;
	}
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
		...
		if (horizontalAmount != 0.0) {
			horizontalScroll = MathHelper.clamp(horizontalScroll - (int)(horizontalAmount * 12), 0, maxHorizontalScroll());
			return true;
		}
		firstVisibleLine = MathHelper.clamp(firstVisibleLine - (int)Math.signum(verticalAmount), 0, maxScrollLine());
		return true;
	}
	...
	public boolean keyPressed(KeyInput input) {
		...
		if (EMUtilsKeyInputs.hasCtrlOrCmd()) {
			if (input.key() == InputUtil.GLFW_KEY_Z) {
				if (input.hasShift()) {
					redo();
				} else {
					undo();
			...
			if (input.key() == InputUtil.GLFW_KEY_Y) {
				redo();
				return true;
			}
		...
		return switch (input.key()) {
			...
			case InputUtil.GLFW_KEY_LEFT -> {
				moveHorizontal(-1, input.hasShift());
				yield true;
			}
			...
			case InputUtil.GLFW_KEY_HOME -> {
				setCaret(caretLine, 0, input.hasShift());
				yield true;
			}
			case InputUtil.GLFW_KEY_END -> {
				setCaret(caretLine, lines.get(caretLine).length(), input.hasShift());
				yield true;
			}
			case InputUtil.GLFW_KEY_PAGE_UP -> {
				moveVertical(-visibleLineCount(), input.hasShift());
				yield true;
			}
			case InputUtil.GLFW_KEY_PAGE_DOWN -> {
				moveVertical(visibleLineCount(), input.hasShift());
				yield true;
			}
			case InputUtil.GLFW_KEY_TAB -> {
				insertText(input.hasShift() ? "" : "\t");
				yield true;
			}
			default -> false;
		};
	...
	public boolean charTyped(CharInput input) {
		if (!isFocused() || !input.isValidChar() || input.codepoint() == '\t') {
			return false;
		}
	...
	private void moveVertical(int amount, boolean selecting) {
		int targetLine = MathHelper.clamp(caretLine + amount, 0, lines.size() - 1);
	...
	private void moveCaretToMouse(double mouseX, double mouseY, boolean selecting) {
		...
		int column = 0;
		while (column < text.length() && textRenderer.getWidth(text.substring(0, column + 1)) < localX) {
			column++;
		}
	...
	private int lineAtMouseY(double mouseY) {
		return MathHelper.clamp(firstVisibleLine + (int)((mouseY - getY() - PADDING) / LINE_HEIGHT), 0, lines.size() - 1);
	}
	...
	private int getScrollbarThumbHeight() {
		...
		return MathHelper.clamp(trackHeight * trackHeight / contentHeight, 8, trackHeight);
	}
	...
	private void scrollToMouseY(double mouseY) {
		...
		int trackHeight = scrollbarTrackHeight();
		int thumbHeight = getScrollbarThumbHeight();
		double ratio = MathHelper.clamp((mouseY - scrollbarTrackTop() - thumbHeight / 2.0) / (trackHeight - thumbHeight), 0.0, 1.0);
		firstVisibleLine = MathHelper.clamp((int)Math.round(ratio * maxScroll), 0, maxScroll);
	}
	...
	private void drawScrollbar(DrawContext context, int mouseX, int mouseY) {
		...
		context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, SCROLLER_BACKGROUND_TEXTURE, x, trackTop, SCROLLBAR_WIDTH, trackHeight);
		context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, SCROLLER_TEXTURE, x, thumbY, SCROLLBAR_WIDTH, thumbHeight);
	}

	private void setCaret(int line, int column, boolean selecting) {
		caretLine = MathHelper.clamp(line, 0, lines.size() - 1);
		caretColumn = MathHelper.clamp(column, 0, lines.get(caretLine).length());
		if (!selecting) {
	...
	private void ensureCaretVisible() {
		...
		int caretPixel = textRenderer.getWidth(lines.get(caretLine).substring(0, caretColumn));
