...
public final class InventoryToolsSettingsScreen extends EMUtilsScreen {
	...
	private static ButtonWidget sortSpeedButton() {
		return ButtonWidget.builder(sortSpeedMessage(), button -> {
	...
	private static Text sortSpeedMessage() {
		return Text.translatable(
			EMUtilsTexts.OPTION_VALUE,
			Text.translatable(EMUtilsTexts.OPTION_SORT_SPEED),
			Text.translatable(EMUtilsClient.config().sortSpeed().labelKey())
		);
	}
}
