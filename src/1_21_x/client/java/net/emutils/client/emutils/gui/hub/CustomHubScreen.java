...
import net.emutils.client.emutils.util.EMUtilsKeyInputs;
...
public final class CustomHubScreen extends Screen {
	...
	public CustomHubScreen(Screen parent) {
		...
		String version = FabricLoader.getInstance()
			...
			.orElse("unknown");
		this.versionText = Text.literal("EMUtils v" + version);
	}
	...
	private void layout() {
		...
		doneHeight = 22;
		doneWidth = textRenderer.getWidth(ScreenTexts.DONE) + 18;
	...
	private void renderPanel(DrawContext context, int mouseX, int mouseY) {
		...
		Text brand = Text.translatable(EMUtilsTexts.NAME);
		drawCenteredScaledText(
			context,
			brand,
			panelX + SIDEBAR_WIDTH / 2,
			panelY + Math.round((HEADER_HEIGHT - textRenderer.fontHeight * BRAND_SCALE) / 2.0F),
			BRAND_SCALE,
			HubPanelTheme.TEXT_ACCENT
		);
		renderPillButton(context, doneX, doneY, doneWidth, doneHeight, ScreenTexts.DONE, contains(mouseX, mouseY, doneX, doneY, doneWidth, doneHeight));
		context.drawText(
			textRenderer,
			versionText,
			panelX + Math.max(0, (SIDEBAR_WIDTH - textRenderer.getWidth(versionText)) / 2),
	...
	private void renderSidebar(DrawContext context, int mouseX, int mouseY) {
		...
		for (FeatureGroup group : FeatureGroup.values()) {
			...
			drawClippedText(
				context,
				Text.translatable(group.labelKey()),
				sidebarX + 32,
				y + (HubPanelTheme.ROW_HEIGHT - textRenderer.fontHeight) / 2,
				sidebarWidth - 38,
				color
			);
	...
	private void renderSearch(DrawContext context) {
		...
		Text display = search.isEmpty() && searchFocused
			? Text.empty()
			: search.isEmpty()
			? Text.translatable(EMUtilsTexts.HUB_SEARCH_PLACEHOLDER)
			: Text.literal(textRenderer.trimToWidth(search, searchWidth - 48));
		context.drawText(
			textRenderer,
			display,
			searchX + 34,
			searchY + (SEARCH_HEIGHT - textRenderer.fontHeight) / 2 + 1,
		...
		if (search.isEmpty() && !searchFocused) {
			...
			Text keybindText = Text.literal(keybind);
			int keybindWidth = textRenderer.getWidth(keybindText);
			context.drawText(
				textRenderer,
				keybindText,
		...
		if (searchFocused && (System.currentTimeMillis() / 500L) % 2L == 0L) {
			String visibleSearch = textRenderer.trimToWidth(search, searchWidth - 48);
			int cursorX = Math.min(searchX + searchWidth - 10, searchX + 34 + textRenderer.getWidth(visibleSearch));
			context.fill(cursorX, searchY + 7, cursorX + 1, searchY + SEARCH_HEIGHT - 7, HubPanelTheme.TEXT_ACCENT);
		}
	}

	private void renderFeatureList(DrawContext context, int mouseX, int mouseY) {
		List<FeatureSpec> visible = visibleFeatures();
		if (visible.isEmpty()) {
			Text empty = Text.translatable(EMUtilsTexts.HUB_EMPTY_SEARCH);
			context.drawText(
				textRenderer,
				empty,
				contentX + (contentWidth - textRenderer.getWidth(empty)) / 2,
				contentY + 24,
	...
	private void renderFeatureRow(DrawContext context, FeatureSpec feature, int y, int mouseX, int mouseY) {
		...
		drawClippedText(context, Text.literal(titleText), contentX + 50, y + 9, textWidth, HubPanelTheme.TEXT_PRIMARY);
		String description = textRenderer.trimToWidth(
			Text.translatable(feature.descriptionKey()).getString(),
			textWidth
		);
		context.drawText(textRenderer, Text.literal(description), contentX + 50, y + 27, HubPanelTheme.TEXT_MUTED, false);
		...
		if (feature.primaryAction() != null) {
			...
			Text actionLabel = Text.translatable(actionEnabled ? EMUtilsTexts.HUB_ACTION_OPEN : EMUtilsTexts.HUB_ACTION_UNAVAILABLE);
			...
			Text label = Text.literal(expanded ? "Hide" : "Settings");
			int width = textRenderer.getWidth(label) + 16;
	...
	private void renderSettingsMenu(DrawContext context, FeatureSpec feature, int y, int height, int mouseX, int mouseY) {
		...
		int renderScroll = MathHelper.clamp(feature == expandedFeature ? (int) Math.round(settingsScroll) : 0, 0, renderMaxScroll);
	...
	private void renderSettingRows(DrawContext context, FeatureSpec feature, int x, int y, int width, int mouseX, int mouseY) {
		for (HubSettingRow row : rows(feature)) {
			...
			if (row instanceof HubSettingRow.Action action) {
				drawClippedText(context, action.label(), x, y + (SETTING_ROW_HEIGHT - textRenderer.fontHeight) / 2, width - ACTION_BUTTON_WIDTH - 12, HubPanelTheme.TEXT_PRIMARY);
				Text actionLabel = Text.translatable(action.enabled() ? EMUtilsTexts.HUB_ACTION_OPEN : EMUtilsTexts.HUB_ACTION_UNAVAILABLE);
				renderPillButton(
					context,
			...
			Text label = settingLabel(row);
			drawClippedText(context, label, x, y + (SETTING_ROW_HEIGHT - textRenderer.fontHeight) / 2, labelWidth - 8, HubPanelTheme.TEXT_PRIMARY);
			if (row instanceof HubSettingRow.Toggle toggle) {
				...
				Text value = sliderValue(slider);
				int valueWidth = Math.min(VALUE_COLUMN_WIDTH, textRenderer.getWidth(value));
				int trackW = Math.max(120, controlWidth - VALUE_COLUMN_WIDTH - 18);
				int trackX = controlX;
				renderSlider(context, slider, trackX, y, trackW);
				drawClippedText(context, value, controlRight - valueWidth, y + (SETTING_ROW_HEIGHT - textRenderer.fontHeight) / 2, valueWidth, HubPanelTheme.TEXT_MUTED);
			} else if (row instanceof HubSettingRow.Cycle<?> cycle) {
				Text value = cycle.valueLabel().get();
				int pillW = Math.min(controlWidth, textRenderer.getWidth(value) + 16);
	...
	private void renderSettingsMenuSurface(DrawContext context, int left, int top, int right, int bottom) {
	...
	private void renderToggle(DrawContext context, boolean enabled, int x, int y) {
	...
	private void renderSlider(DrawContext context, HubSettingRow.Slider slider, int x, int y, int width) {
		int trackHeight = 8;
		...
		int fillRight = x + (int) Math.round(width * MathHelper.clamp(ratio, 0.0, 1.0));
		if (fillRight > x) {
			drawCapsule(context, x, trackY, Math.max(x + trackHeight, fillRight), trackY + trackHeight, HubPanelTheme.ACCENT);
		}
		int thumbX = MathHelper.clamp(fillRight, x + trackHeight / 2, x + width - trackHeight / 2);
		HubRoundedGraphics.drawCircle(context, thumbX, trackY + trackHeight / 2, 14, HubPanelTheme.TEXT_PRIMARY);
	}

	private void renderPillButton(DrawContext context, int x, int y, int width, int height, Text label, boolean hovered) {
		renderPillButton(context, x, y, width, height, label, hovered, true);
	}

	private void renderPillButton(DrawContext context, int x, int y, int width, int height, Text label, boolean hovered, boolean enabled) {
		int background = enabled
			? hovered ? HubPanelTheme.SURFACE_HOVER : HubPanelTheme.SURFACE
			: 0xFF1B2434;
		String visible = textRenderer.trimToWidth(label.getString(), Math.max(0, width - 8));
		int labelWidth = textRenderer.getWidth(visible);
		...
		context.drawText(
			...
			y + (height - textRenderer.fontHeight + 1) / 2,
			enabled ? HubPanelTheme.TEXT_ACCENT : HubPanelTheme.TEXT_MUTED,
			false
		);
	}

	private void renderBottomFade(DrawContext context) {
	...
	private void renderSettingsBottomFade(DrawContext context, int x, int y, int width, int height) {
	...
	private void drawCenteredScaledText(DrawContext context, Text text, int centerX, int y, float scale, int color) {
		int textWidth = textRenderer.getWidth(text);
		context.getMatrices().pushMatrix();
		try {
			context.getMatrices().translate(centerX - textWidth * scale / 2.0F, y);
	...
	private void drawClippedText(DrawContext context, Text text, int x, int y, int maxWidth, int color) {
		if (maxWidth <= 0) {
			return;
		}
		context.drawText(textRenderer, Text.literal(textRenderer.trimToWidth(text.getString(), maxWidth)), x, y, color, false);
	}
	...
	private void renderMiniScrollbar(DrawContext context, int x, int y, int height, int scroll, int max) {
		if (max <= 0) {
			return;
		}
	...
	public boolean mouseClicked(Click click, boolean doubled) {
		...
		if (handleFeatureClick(click.x(), click.y())) {
			return true;
		}

		if (handleSettingClick(click.x(), click.y())) {
			return true;
		}
	...
	private boolean handleFeatureClick(double mouseX, double mouseY) {
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
		...
		if (menu != null && settingsMaxScroll > 0 && contains(mouseX, mouseY, menu.x(), menu.y(), menu.width(), menu.height())) {
			suppressSettingsBottomFade = false;
			settingsScrollTarget = MathHelper.clamp(settingsScrollTarget - verticalAmount * 18.0, 0.0, settingsMaxScroll);
			return true;
		}
		if (maxScroll > 0 && contains(mouseX, mouseY, contentX, contentY, contentWidth, contentHeight)) {
			suppressListBottomFade = false;
			suppressSettingsBottomFade = false;
			scrollOffsetTarget = MathHelper.clamp(scrollOffsetTarget - verticalAmount * 22.0, 0.0, maxScroll);
			return true;
		}
	...
	public boolean keyPressed(KeyInput input) {
		if (!searchFocused) {
			if (EMUtilsKeyInputs.hasCtrlOrCmd() && input.key() == InputUtil.GLFW_KEY_F) {
				searchFocused = true;
				return true;
			}
	...
	public boolean charTyped(CharInput input) {
		if (!searchFocused || !input.isValidChar() || input.codepoint() == '\t') {
	...
	private void updateScrollBounds() {
		...
		scrollOffsetTarget = MathHelper.clamp(scrollOffsetTarget, 0.0, maxScroll);
		scrollOffset = MathHelper.clamp(scrollOffset, 0.0, maxScroll);
		...
		settingsScrollTarget = MathHelper.clamp(settingsScrollTarget, 0.0, settingsMaxScroll);
		settingsScroll = MathHelper.clamp(settingsScroll, 0.0, settingsMaxScroll);
	}
	...
	private static int menuHeightForProgress(int height, float progressValue) {
		...
		float clamped = MathHelper.clamp(progressValue, 0.0F, 1.0F);
	...
	private static int settingLabelWidth(int width) {
		return MathHelper.clamp(width / 2 - 12, 150, 220);
	}
	...
	private void applySlider(HubSettingRow.Slider slider, int x, int width, double mouseX) {
		double ratio = MathHelper.clamp((mouseX - x) / width, 0.0, 1.0);
	...
	private Text settingLabel(HubSettingRow row) {
		if (row instanceof HubSettingRow.Toggle toggle) {
			return Text.translatable(toggle.labelKey());
		}
		if (row instanceof HubSettingRow.Slider slider) {
			return Text.translatable(slider.labelKey());
		}
		if (row instanceof HubSettingRow.Cycle<?> cycle) {
			return Text.translatable(cycle.labelKey());
		}
		if (row instanceof HubSettingRow.Rgb rgb) {
			return Text.translatable(rgb.labelKey());
		}
		return Text.empty();
	}

	private Text sliderValue(HubSettingRow.Slider slider) {
		Text suffix = slider.suffixKey() == null || slider.suffixKey().isEmpty()
			? Text.empty()
			: Text.translatable(slider.suffixKey());
		return Text.literal(String.valueOf(slider.getter().getAsInt())).append(suffix);
	}
	...
	private static void drawIcon(DrawContext context, Identifier texture, int x, int y, int size) {
		context.drawTexture(RenderPipelines.GUI_TEXTURED, texture, x, y, 0.0F, 0.0F, size, size, 32, 32, 32, 32);
	}

	private static void drawCapsule(DrawContext context, int left, int top, int right, int bottom, int color) {
	...
	private static String cleanTitle(Text title) {
	...
	private HubSettingRow.Action openAction(String labelKey, Function<Screen, Screen> screenFactory, boolean enabled) {
		return new HubSettingRow.Action(Text.translatable(labelKey), openScreenAction(screenFactory), enabled);
	}
	...
	private record FeatureSpec(
		...
		private boolean matches(String query) {
			return normalized(Text.translatable(titleKey).getString()).contains(query)
				|| normalized(Text.translatable(descriptionKey).getString()).contains(query);
		}
	}
}
