...
public final class EMUtilsHubScreen extends EMUtilsScreen {
	...
	protected void initBody() {
		...
		adder.addChild(fullWidthSettingsButton(Component.translatable(EMUtilsTexts.HUB_MODERN_OPEN), button -> client.gui.setScreen(SettingsScreens.hub(this))), SETTINGS_COLUMNS);
	...
	private Button navButton(String labelKey, BooleanSupplier enabled, Function<Screen, Screen> screenFactory) {
		Button button = Button.builder(statusLabel(labelKey, enabled.getAsBoolean()), ignored -> client.gui.setScreen(screenFactory.apply(this))).build();
