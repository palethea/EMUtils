...
public final class HubRoundedGraphics {
	...
	public static void drawPill(GuiGraphicsExtractor context, int left, int top, int right, int bottom, int color) {
		if (right <= left || bottom <= top) {
			return;
		}
	...
	public static void drawCircle(GuiGraphicsExtractor context, int centerX, int centerY, int diameter, int color) {
		if (diameter <= 0) {
			return;
		}
	...
	private static void ensureCacheScale(Minecraft client) {
	...
	private static CachedPatch ensureRectPatch(Minecraft client, int color) {
		ensureCacheScale(client);
	...
	private static CachedPatch ensurePillPatch(Minecraft client, int color, int screenHeight) {
		ensureCacheScale(client);
		int ss = activeSupersample;
	...
	private static CachedPatch ensureCirclePatch(Minecraft client, int color, int screenDiameter) {
		ensureCacheScale(client);
		int ss = activeSupersample;
	...
	private static CachedPatch bakePillPatch(Minecraft client, int color, int logicalHeight, int supersample) {
	...
	private static CachedPatch bakeCirclePatch(Minecraft client, int color, int screenDiameter, int supersample) {
		...
		for (int y = 0; y < texSize; y++) {
			for (int x = 0; x < texSize; x++) {
				...
				if (pixelAlpha <= 0) {
					...
					image.setPixel(x, y, (pixelAlpha << 24) | (red << 16) | (green << 8) | blue);
				}
			}
		}
	...
	private static CachedPatch register(
		...
		Identifier id = Identifier.fromNamespaceAndPath(EMUtilsClient.MOD_ID, "hub_rounded/" + kind + "/" + PATCH_CACHE.size());
		DynamicTexture texture = new DynamicTexture(() -> "EMUtils hub UI " + kind, image);
