...
public final class UiLoadingOverlay extends Overlay {
	...
	/** The pack's icon texture and its pixel size, or just {@code symbol} when it has no icon (#115). */
	public record Icon(Identifier symbol, @Nullable Identifier texture, int width, int height) {
		public static Icon symbol(Identifier symbol) {
			return new Icon(symbol, null, 0, 0);
		}
	}
	...
	private UiLoadingOverlay(Minecraft minecraft, ReloadInstance reload, Consumer<Optional<Throwable>> onFinish, Component title, Component subtitle, Icon icon, @Nullable Runnable start) {
	...
	public static UiLoadingOverlay running(Minecraft minecraft, ReloadInstance reload, Consumer<Optional<Throwable>> onFinish, Component title, Component subtitle, Icon icon) {
		return new UiLoadingOverlay(minecraft, reload, onFinish, title, subtitle, icon, null);
	}

	/** A card for a reload that blocks the game while it runs, such as a shader compile; it's started once the card is shown. */
	public static UiLoadingOverlay blocking(Minecraft minecraft, ReloadInstance reload, Runnable start, Consumer<Optional<Throwable>> onFinish, Component title, Component subtitle, Icon icon) {
		return new UiLoadingOverlay(minecraft, reload, onFinish, title, subtitle, icon, start);
	}
	...
	public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
		...
		// The pack's icon sits left of the title, name and bar, centered on them.
		int block = titleHeight + 7 + lineHeight + 14 + BAR_HEIGHT;
		drawIcon(context, theme, x + PADDING, y + PADDING + (block - ICON) / 2, alpha);
		int left = x + PADDING + ICON + ICON_GAP;
		int textWidth = WIDTH - PADDING * 2 - ICON - ICON_GAP;
	...
	/** The pack's icon in a rounded tile, drawn like its row in the Pack Manager, or its symbol when it has none. */
	private void drawIcon(GuiGraphicsExtractor context, UiTheme theme, int x, int y, float alpha) {
		UiShapes.roundedRect(context, x, y, ICON, ICON, 8, theme.segmentBackground());
		if (icon.texture() != null && icon.width() > 0 && icon.height() > 0) {
			...
			context.pose().scale(scale, scale);
			context.blit(RenderPipelines.GUI_TEXTURED, icon.texture(), 0, 0, 0.0F, 0.0F, icon.width(), icon.height(), icon.width(), icon.height(), icon.width(), icon.height(), UiTheme.fade(0xFFFFFFFF, alpha));
			context.pose().popMatrix();
			return;
		}
		int symbol = 18;
		UiIcons.draw(context, icon.symbol(), x + (ICON - symbol) / 2, y + (ICON - symbol) / 2, symbol, theme.muted());
	}
